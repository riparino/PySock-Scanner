...
nm.scan('127.0.0.1', '22-443')      # scan host 71.59.0.201, ports from 22 to 443
nm.command_line()                   # get command line used for the scan : nmap -oX - -p 22-443 71.59.0.201
nm.scaninfo()                       # get nmap scan informations {'tcp': {'services': '22-443', 'method': 'connect'}}
nm.all_hosts()                      # get all hosts that were scanned
nm['127.0.0.1'].hostname()          # get one hostname for host 71.59.0.201, usualy the user record
nm['127.0.0.1'].hostnames()         # get list of hostnames for host 127.0.0.1'name':'hostname1', 'type':'PTR'}, {'name':'hostname2', 'type':'user'}]
nm['127.0.0.1'].state()             # get state of host 71.59.0.201 (up|down|unknown|skipped) 
...
if nm['127.0.0.1'].has_tcp(22):     # is there any information for port 22/tcp on host 71.59.0.201
    nm['127.0.0.1']['tcp'][22]          # get infos about port 22 in tcp on host 71.59.0.201
    nm['127.0.0.1'].tcp(22)             # get infos about port 22 in tcp on host 71.59.0.201
    nm['127.0.0.1']['tcp'][22]['state'] # get state of port 22/tcp on host 71.59.0.201 (open
